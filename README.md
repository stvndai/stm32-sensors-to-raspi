...
Frontend: HTML/JS (live dashboard) — see Roadmap for planned React upgrade
Wiring
...
Flash to the Nucleo-F446RE
Confirm readings over the debug UART (USART2)
Raspberry Pi
Enable I2C:
bash
   sudo raspi-config
...
Confirm the STM32 is visible on the bus:
bash
   i2cdetect -y 1
...
Roadmap
 Rebuild the dashboard in React + Recharts (matching the styling of my Lichess Player Tracker)
